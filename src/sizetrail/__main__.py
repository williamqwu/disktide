...
from sizetrail import APP_NAME, __version__
...
def cli(
    ...
    if ctx.invoked_subcommand is None:
        ...
        app = SizeTrailApp(show_welcome=True, config=config)
        app.run(mouse=False)
...
def scan(
    ...
    if snapshot:
        from sizetrail.repositories import default_snapshot_repository
...
def compare(
    ...
    from sizetrail.config import parse_duration
    from sizetrail.repositories import default_snapshot_repository
    from sizetrail.services.compare import (
        CompareService,
    ...
    if since is not None:
        ...
        if len(selectors) not in {0, 2, 3}:
            raise click.UsageError(
                "use: sizetrail compare TARGET BASELINE [PATH]"
            )
...
def _size_value(value: str, *, param_hint: str) -> int:
    from sizetrail.config import parse_size
...
def monitor_list(include_archived: bool, json_output: bool) -> None:
    ...
    from sizetrail.config import format_duration
    ...
    try:
        ...
        if not dashboard.monitors:
            click.echo("No monitor definitions. Use: sizetrail monitor add PATH")
            return
        click.echo(
...
def monitor_run(identifier: str) -> None:
    ...
    from sizetrail.metrics import METRIC_NAMES, metric_text
...
def alerts_add(
    ...
    from sizetrail.domain.alerts import AlertKind, AlertRule, AlertSeverity
    from sizetrail.domain.metrics import MetricId
...
def alerts_edit(
    ...
    from sizetrail.domain.alerts import AlertKind, AlertSeverity
    from sizetrail.domain.metrics import MetricId
...
def watch(
    ...
    from sizetrail.domain.monitor import MonitorDefinition
    from sizetrail.domain.policy import ScanPolicy
    from sizetrail.services.monitor import MonitorEventKind
...
def cleanup(
    ...
    from sizetrail.cleanup.rules import get_rule_by_name, get_rule_catalog
    ...
    from sizetrail.domain.metrics import MetricId
    from sizetrail.domain.policy import ScanPolicy
    from sizetrail.domain.scan import ScanRequest, ScanRequestError, ScanStatus
    from sizetrail.repositories import default_snapshot_repository
    ...
    if arguments and arguments[0] == "rules":
        ...
        if len(arguments) != 2 or plan_id or apply_safe or permanent:
            raise click.UsageError("usage: sizetrail cleanup undo PLAN_OR_ACTION_ID")
        operation = "undo"
        operand = arguments[1]
    elif arguments and arguments[0] == "purge":
        if len(arguments) != 2 or plan_id or apply_safe or permanent or history_group:
            raise click.UsageError("usage: sizetrail cleanup purge PLAN_OR_ACTION_ID")
        operation = "purge"
        operand = arguments[1]
        ...
        if (
            ...
            raise click.UsageError(
                "usage: sizetrail cleanup quarantine audit|rebuild ROOT"
            )
    ...
    try:
        ...
        if not apply_safe and not permanent:
            if not json_output:
                click.echo(
                    ...
                    f"sizetrail cleanup --plan {plan.id} --apply"
                )
            return
        ...
        if json_output:
            ...
            if any(action.undo_available for action in result.plan.actions):
                click.echo(f"  Undo: sizetrail cleanup undo {result.plan.id}")
