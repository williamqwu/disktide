...
from sizetrail.config import AppConfig, format_duration, save_config
from sizetrail.domain.alerts import AlertRule
...
from sizetrail.domain.visualization import MonitorSpaceTime
...
from sizetrail.presentation.tui.viewmodels.visualization import legend_text
...
class MonitorScreen(Screen):
    ...
    def _render_empty(self) -> None:
        ...
        self.query_one("#monitor-history-summary", Static).update(
            "[b]No monitor definition covers the current selection.[/b]\n"
            f"{action}\n"
            "Saved definitions need this TUI session or sizetrail watch --all."
        )
    ...
    def _history_collection_line(summary: MonitorSummary) -> str:
        ...
        if status.activity is MonitorActivityState.NO_HOST:
            return (
                ...
                "press s or run sizetrail watch --all"
            )
        next_due = (
