...
from sizetrail.domain.alerts import (
    AlertEvent,
    AlertKind,
    AlertRule,
    AlertSeverity,
...
from sizetrail.domain.delta import NodeMeasurement, SizeDelta
...
from sizetrail.domain.provisional import ProvisionalSummary
from sizetrail.domain.snapshot import (
    Snapshot,
    policy_from_dict,
    policy_to_dict,
)
...
from sizetrail.paths import database_file
...
class Database:
    """SQLite-backed storage for SizeTrail data."""
    ...
    def connect(self) -> None:
        ...
        self.recovery_hint = (
            ...
            "database copy; sizetrail will not delete the file automatically"
        )
    ...
    def _row_to_snapshot(self, row) -> Snapshot:
        if len(row) <= 8:
            ...
            log.warning(
                ...
                "sizetrail once with migrations enabled).",
                row[0],
            )
        ...
        *,
        ...
        for node in tuple(nodes.values()):
            ...
            node.children.append(
                FSNode(
                    name="… remainder",
                    path=f"{node.path.rstrip('/')}/.sizetrail-projection-remainder",
                    size=logical,
                    own_size=logical,
