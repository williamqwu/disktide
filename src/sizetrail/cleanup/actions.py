...
from sizetrail import QUARANTINE_DIRECTORY_NAME
...
from sizetrail.models.patterns import CleanupTarget, RiskLevel
...
def _require_resolved_ledger(status: QuarantineLedgerStatus) -> None:
    if status.issues:
        raise CleanupExecutionError(
            ...
            f"'sizetrail cleanup quarantine audit {status.root}'"
        )
