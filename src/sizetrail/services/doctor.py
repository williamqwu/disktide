...
from sizetrail.cleanup.actions import QuarantineExecutor, mutation_capabilities
...
from sizetrail.config import AppConfig, load_config
from sizetrail.extensions.capabilities import (
    Capability,
    CapabilityId,
    CapabilityStatus,
)
from sizetrail.paths import cache_root, config_file, data_root, state_log_file
from sizetrail.scanner.sysinfo import detect_cpu_count
from sizetrail.storage.database import Database
from sizetrail.storage.migrations import CURRENT_VERSION, get_version
    ...
    *,
    ...
    return {
        ...
        "quarantine_ledger": {
            ...
            "roots": statuses,
            "issues": issues,
            "rebuild_command": "sizetrail cleanup quarantine rebuild ROOT",
        },
    }
...
def _safe_memory_probe(adapter: PlatformAdapter):
    try:
        ...
        from sizetrail.collectors.platform.models import ProbeResult
...
def _optional_extras(configured_mode: str) -> dict[str, object]:
    from sizetrail.collectors.events.native import probe_native_event_backend
    ...
    watch.update(
        {
            ...
            "install": "uv tool install 'sizetrail[watch]'",
        }
    )
