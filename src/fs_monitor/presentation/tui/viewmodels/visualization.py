"""Unified glyphs, labels, and formatting for space-time visualizations."""
...
from fs_monitor.rendering import is_safe_rendering
...
def visual_token(state: VisualState) -> VisualToken:
    ...
    if is_safe_rendering():
        return VisualToken(token.label, token.safe_glyph, token.safe_glyph, token.color)
    return token
