...
from fs_monitor.screens.fs_overview import (
    FSOverviewScreen,
    FSDetailModal,
    _format_benchmark,
)
from fs_monitor.scanner.benchmark import BenchmarkResult
...
def test_double_b_confirms_records_and_displays(tmp_path):
    ...
    async def go():
        app = _new_app(tmp_path)
        with patch(
            "fs_monitor.screens.fs_overview.benchmark_mount", return_value=_FAKE
        ):
            async with app.run_test(size=(120, 40)) as pilot:
                screen = await _wait_for_overview(pilot, app)
