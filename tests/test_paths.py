...
from sizetrail.config import cleanup_rule_directory, config_path
from sizetrail.paths import database_file, state_log_file
...
def test_new_install_uses_sizetrail_namespace(tmp_path, monkeypatch):
    _set_xdg_roots(monkeypatch, tmp_path)

    assert config_path() == tmp_path / "config" / "sizetrail" / "config.toml"
    assert cleanup_rule_directory() == (
        tmp_path / "config" / "sizetrail" / "cleanup-rules"
    )
    assert database_file() == tmp_path / "data" / "sizetrail" / "data.db"
    assert state_log_file() == (
        tmp_path / "state" / "sizetrail" / "sizetrail.log"
    )


def test_existing_legacy_paths_remain_active(tmp_path, monkeypatch):
    _set_xdg_roots(monkeypatch, tmp_path)
...
def test_sizetrail_paths_win_when_both_namespaces_exist(tmp_path, monkeypatch):
    _set_xdg_roots(monkeypatch, tmp_path)
    for namespace in ("sizetrail", "fsmonitor-cli"):
