...
- **fix** Symlink scanning is now one syscall per entry, matching v0.1.3. The walker previously did three syscalls per symlink (`entry.stat` for the link, `os.readlink` for the target text, `os.stat` to follow the link and classify the target type); on a cluster home with millions of symlinks under shared image caches (e.g. a `.dataset/` tree pointing into a ZTF cutout cache) every extra syscall was a server round-trip, so the same scan that finished in 244 s on v0.1.3 took 2386 s here. The walker now pays only `entry.stat(follow_symlinks=False)` while scanning; `os.readlink` and the target-following `os.stat` are deferred to `classify_symlink`, which the Details panel and the `i` action call on demand for the one symlink the user is actually looking at. The tree's `→ /target` arrow, the trailing `/` on a dir-target, and the `(broken)` marker appear for symlinks the user has selected (which classifies them and caches the result); symlinks they have not yet visited show just the name in cyan.
