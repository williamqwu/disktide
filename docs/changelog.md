...
- **fix** Symlink scanning is now one syscall per entry, matching v0.1.3. The walker previously did three syscalls per symlink (`entry.stat` for the link, `os.readlink` for the target text, `os.stat` to follow the link and classify the target type); on a cluster home with millions of symlinks under shared image caches (e.g. a `.dataset/` tree pointing into a Zwicky Transient Facility cutout cache) every extra syscall was a server round-trip, so the same scan that finished in 244 s on v0.1.3 took 2386 s here. The walker now pays only `entry.stat(follow_symlinks=False)` while scanning; `os.readlink` and the target-following `os.stat` are deferred to `classify_symlink`, which the Details panel and the `i` action call on demand for the one symlink the user is actually looking at. After the fix the same scan completes in 195 s (~12x recovery, ~20% under the v0.1.3 baseline).
- **feat** Engine eagerly classifies the first 100 symlinks at the scan root so a typical `fsmon ~` still renders inline `→ /target` arrows for the few links at home root without re-introducing the per-symlink cost when the scan root itself contains hundreds of thousands of symlinks. Deeper symlinks remain fully lazy and are filled in on demand when the user selects them.
