...
- The progress overlay is positioned with `position: absolute` and `layer: overlay`, claiming only its own 60x12 cells. An earlier full-screen wrapper container with `background: transparent` did NOT pass through to the lower layer: Textual's compositor treats any widget covering a cell as owning that cell, regardless of background alpha, so the live sunburst was being painted into its strip and then occluded by the wrapper. `_center_overlay()` on the explorer sets the overlay's offset to keep it centered, and `on_resize` re-runs it on terminal resize.
