...
Five built-in schemes: `disktide` (the default), `cold`, `colorblind`, `cyberpunk`, `mono`. A `ColorScheme` is a set of table keys, not a set of colors — `name`, `label` (the picker text), `category_key`, `neutral_key`, `delta_key`, `textual_theme`, and the treemap's `border_bg`/`dir_leaf_bg`.

Every theme has two halves. The **charts** read `viz/colors.py`, which imports no Textual because the renderers run off the UI thread during a live scan. The **chrome** — header, borders, selection, footer, panel backgrounds — is a Textual `Theme` in `viz/chrome.py`, registered on `DiskTideApp.__init__` and applied by `App.theme`. `disktide` names the built-in `textual-dark` rather than carrying a copy, so it is pixel-identical to what shipped before the rename. `DiskTideApp.apply_color_theme(name)` is the only place that moves both halves; `set_color_scheme` bumps the render epoch for the charts and setting `App.theme` refreshes the CSS for the chrome. Under `NO_COLOR` the Textual theme is left alone, because the app is already in ansi mode.

The vocabulary — which theme keys exist, and what a retired name resolves to — lives in `disktide/themes.py`, a module with no imports. `config.py` needs it and `viz/colors.py` costs ~70 ms to import (it reaches the whole domain graph through `VisualState`), which is most of a `disktide scan` that only wanted to read a config file. `warm`, `default` and `vivid` all resolve to `disktide`.

Category colors *are* per-scheme, as of the five-theme redesign. Until then one shared table was used and a theme only re-tempered the neutrals, on the argument that a category should mean one colour everywhere; that argument does not survive a cold theme whose every hue is cool, or a colorblind-safe theme separated by lightness rather than hue. What is kept instead is the hue *order*: `ephemeral` sits at the warm end and `docs`/`archive` at the cool end in all four chromatic themes, so the chart still reads the same way after a theme change. Seven content types (code, docs, data, media, archive, ephemeral, other) are colored from precomputed OKLCH-derived tables in `viz/colors.py`, generated by `tool/gen_palette.py`, which holds the per-theme anchors and lightness ladders as the single source of truth. Eleven categories could not be given hues that stayed pairwise distinguishable under colorblind simulation; six could, so pairs that call for the same cleanup decision were merged.

`mono` is achromatic and carries `category_key = None`: its six categories come from `MONO_FILE_RGB`, a gray lightness ladder whose every step is above the shallowest directory neutral, and it never tints a directory — a gray tint would only make a directory look like a deeper directory.

Coloured **text** is a third thing, separate from both the chart fills and the Textual chrome. The tree, the breadcrumb, the scan progress panel, the details table, the cleanup views and FS-Overview all print styled `Text`, and each of them used to name a Rich colour directly (`bold cyan` directories, a `green` size bar, `bold red` errors) — fixed sRGB values no theme could reach, which is why `mono` drew a neon-green bar and cyan directory names. `ColorScheme.inks` maps fifteen semantic roles (`dir`, `file`, `link`, `link_dim`, `crumb`, `muted`, `bar`, `bar_strong`, `warning`/`_strong`/`_dim`, `error`/`_strong`, `accent`/`_dim`) to Rich style strings, and `viz.colors.ink(role)` resolves one against the active scheme. `_ink_table()` composes the bold/dim/underline variants from eight base colours, which is what lets `disktide` reproduce its legacy strings exactly by passing Rich's colour *names* — there is no separate legacy table to drift from. `scanner/sysinfo.py`'s `storage_class` and `facet_labels` return role names rather than colours for the same reason: that layer has no business knowing what "slow" looks like. Every ink is measured at ≥ 4.5:1 (the WCAG floor for text, stricter than the 3:1 a chart fill needs) against its own theme's surface, and `mono`'s fifteen are asserted to carry zero chroma.

Two Textual behaviours have to be worked around for a theme to reach the whole window. A widget that overrides `render_line` hands back its own `Strip`, which is composited verbatim: `StylesCache.render_line` only paints the parts of a line *it* generates, so any segment a chart widget built without a bgcolor reached the terminal as SGR 49 — the emulator's default background, not the theme's. `widgets/OpaqueStripMixin` renames those widgets' method to `render_content_line` and lays the widget's own `rich_style` under the result; `#viz-panel` names its `$background` explicitly so the mixin and `SunburstView._panel_bg` cannot drift. And Textual's `Tree` stores each label as a `Text` built once by `_make_label` and handed over with `set_label`, so a `refresh()` redraws the colours it already had; `screens.repaint_widgets` calls a widget's `rebuild_render_cache` before refreshing it, which is how the tree re-themes without a restart. `tests/test_theme_chrome.py` drives every screen in every theme and asserts the compositor emits zero cells without a background.

Each theme's `background`/`surface` pair is held at least 6 OKLab ΔE from every other theme's, and the two chromatic surfaces at least 60° apart in hue; `disktide` and `colorblind` deliberately share #121212/#1e1e1e. Six rather than twelve because every surface also has to stay at or below OKLab L 0.26 — a dark background lives in a small corner of OKLab, and a 12 ΔE floor between five of them can only be bought with saturation or lightness.

The gates the anchors were fitted to are runnable. `tool/palette_checks.py` is a stdlib port of the six-checks data-viz validator (OKLab, Machado-Oliveira-Fernandes 2009 severity-1.0 protan/deutan/tritan simulation, ΔE, WCAG contrast), cross-checked digit for digit against the reference JS implementation in its own docstring. `python tool/gen_palette.py --check` prints every theme's measured numbers, and `tests/test_palette_gates.py` pins them — including that the generator still reproduces the block in `viz/colors.py` byte for byte, that `disktide`'s tables equal the literal values that shipped, and that no two themes are close enough to read as the same theme.

The diff views have three diverging tables, selected by `delta_key`: `default` (shared by `disktide`, `cold` and `cyberpunk`), `colorblind` — whose growth/shrink poles move to the blue/orange axis dichromats keep, because red-against-green measures ΔE 5.0 under deutan simulation — and `mono`, one diverging lightness ramp with the poles at the ends. `NO_COLOR` takes the `mono` table whatever the theme, so the eight states stay apart on the one channel a no-colour terminal has.
...
Helper functions: `file_category(name)` maps extensions to categories; `category_file_color()` / `category_dir_tint()` / `neutral_dir_color()` / `category_legend_color()` resolve table lookups against the active scheme; `scheme_legend_rgb(scheme)` returns a *named* theme's six swatches without making it active, which is what the settings preview draws; `label_ink(background)` picks white or near-black for a label painted straight onto a chart fill, measured rather than assumed, because the colorblind and cyberpunk tables reach OKLab L 0.93 where a white label disappears.
