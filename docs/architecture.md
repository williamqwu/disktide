...
- `ScanEngine` accepts an optional `tree_callback: Callable[[FSNode], None]` and a `tree_callback_interval` (default 1.0 s). The engine fires the callback once after the top-level `scandir` (force), then at most once per interval as each top-level subdir future resolves, and unconditionally one final time at the end. Each emit is a fresh shallow-copy `FSNode` built on the engine thread, so the UI thread reads an immutable handoff.
...
- The user-facing toggle is `ui.live_scan_render`: `auto` (default), `on`, or `off`. `resolve_live_scan_render()` in `config.py` resolves `auto` against `shutil.get_terminal_size()` and `os.cpu_count()`: live mode is on only when the terminal is at least 80 columns by 24 rows AND at least 4 CPUs are available. Below that, the per-frame cost is measurable against the scan and the chart has no room to be visible around the 60x12 progress overlay, so we silently fall back to the v0.1.5 behavior. The Settings screen exposes the same dropdown.
